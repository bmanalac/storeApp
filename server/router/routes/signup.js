  ...
  router.post('/', function(req, res) {
    ...
    Users.findOne({
      ...
      if (err) {
        ...
        // send the error
        res.status(500).json({
          'message': 'Internal server error from signing up new user. Please contact support@gooeycrew.com.'
        });
      }
      ...
      if (!user) {
        ...
        newUser.save(function(err, savedUser, numberAffected) {
          ...
          if (err) {
            ...
            res.status(500).json({
              'message': 'Database error trying to sign up.  Please contact support@gooeycrew.com.'
            });
          }
